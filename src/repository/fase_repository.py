...
async def get_all_fases(conn: connection) -> list[RealDictRow]:
...
async def get_fase_by_id(conn: connection, fase_id: int) -> RealDictRow | None:
    with conn.cursor(cursor_factory=RealDictCursor) as cursor:
        try:
            cursor.execute("""
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as cursor:
        try:
            cursor.execute("""
                        ...
                        """)
            cursor.execute(cursor, (fase.nome, fase.descritivo, fase.ordem))
            return cursor.fetchone()
        except Exception as e:
            print_error_details(e)
# PUT
